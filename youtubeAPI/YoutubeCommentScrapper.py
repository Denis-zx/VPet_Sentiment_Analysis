...
from Senti import extract_video_id  # Assuming Senti is a custom module for video ID extraction
...
DEVELOPER_KEY = st.secrets["API_KEY"]  # Retrieve the API key securely using Streamlit secrets
YOUTUBE_API_SERVICE_NAME = 'youtube'
YOUTUBE_API_VERSION = 'v3'
# Create a client object to interact with the YouTube API
youtube = build(YOUTUBE_API_SERVICE_NAME, YOUTUBE_API_VERSION, developerKey=DEVELOPER_KEY)

# video_id = extract_video_id(youtube_link)  # Assuming you extract the video ID from a link

def get_channel_id(video_id):
    # Retrieve the channel ID associated with the video
    response = youtube.videos().list(part='snippet', id=video_id).execute()
    channel_id = response['items'][0]['snippet']['channelId']
    return channel_id

# channel_id = get_channel_id(video_id)  # Assuming you use the extracted video ID
...
def get_video_stats(video_id):
    try:
        # Retrieve video statistics for the specified video ID
...
def get_channel_info(youtube, channel_id):
    try:
        # Retrieve channel information for the specified channel ID
        ...
        # Store channel information in a dictionary
        channel_info = {
